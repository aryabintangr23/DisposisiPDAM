...
@section('content')
    ...
    <!-- Tab Navigation: Kotak Masuk / Kotak Terkirim -->
    <div class="mb-3 inline-flex rounded-lg border border-slate-200 bg-slate-100 p-1">
        <a href="{{ route('pesan.index', array_filter(['kotak' => 'masuk', 'arah' => $arah])) }}" class="inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-semibold transition {{ $kotak === 'masuk' ? 'bg-white text-brand-700 shadow-sm' : 'text-slate-500 hover:text-slate-700' }}">
            Kotak Masuk
            @if (auth()->user()->jumlahPesanBelumDibaca() > 0)
                <span class="inline-flex h-4 min-w-[1rem] items-center justify-center rounded-full bg-rose-500 px-1 text-[10px] font-bold text-white">
                    {{ auth()->user()->jumlahPesanBelumDibaca() > 9 ? '9+' : auth()->user()->jumlahPesanBelumDibaca() }}
                </span>
            @endif
        </a>
        <a href="{{ route('pesan.index', array_filter(['kotak' => 'keluar', 'arah' => $arah])) }}" class="inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-semibold transition {{ $kotak === 'keluar' ? 'bg-white text-brand-700 shadow-sm' : 'text-slate-500 hover:text-slate-700' }}">
            Kotak Terkirim
        </a>
    </div>
    ...
    <div class="mb-5 flex flex-wrap items-center gap-2">
        <a href="{{ route('pesan.index', ['kotak' => $kotak]) }}" class="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold transition {{ ! $arah ? 'bg-brand-700 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
            Semua
        </a>
        <a href="{{ route('pesan.index', ['kotak' => $kotak, 'arah' => 'masuk']) }}" class="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold transition {{ $arah === 'masuk' ? 'bg-sky-600 text-white' : 'bg-sky-50 text-sky-700 hover:bg-sky-100' }}">
        ...
        <a href="{{ route('pesan.index', ['kotak' => $kotak, 'arah' => 'keluar']) }}" class="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold transition {{ $arah === 'keluar' ? 'bg-violet-600 text-white' : 'bg-violet-50 text-violet-700 hover:bg-violet-100' }}">
    ...
    <form method="POST" action="{{ route('pesan.hapus') }}" x-data="{ selected: [], allIds: {{ $pesan->pluck('id')->map(fn ($id) => (string) $id)->toJson() }} }">
        @csrf

        <!-- Action Bar Saat Ada Pesan Terpilih -->
        <div class="mb-3 flex flex-wrap items-center justify-between gap-2" x-show="selected.length > 0" x-cloak>
            ...
            <div class="flex items-center gap-2">
                @if ($kotak === 'masuk')
                    <button type="submit" formaction="{{ route('pesan.tandaiDibaca') }}" class="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-700">
                ...
                @endif
                <button type="submit" formaction="{{ route('pesan.hapus') }}" onclick="return confirm('Pindahkan pesan yang dipilih ke tempat sampah?')" class="inline-flex items-center gap-2 rounded-lg bg-rose-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-rose-700">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
