        ...
        <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm" x-data="{ arahSurat: '{{ old('arah_surat', $surat->arah_surat->value) }}' }">
            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="text" name="jenis_surat" value="{{ old('jenis_surat', $surat->jenis_surat) }}" required placeholder="Contoh: Surat Undangan" class="w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm text-slate-800 placeholder-slate-400 shadow-sm focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/30">
                </div>

                <div>
                    <label class="mb-1.5 block text-sm font-medium text-slate-700">Nomor Surat</label>
                    <input type="text" id="nomor_surat" name="nomor_surat" value="{{ old('nomor_surat', $surat->nomor_surat) }}" required autocomplete="off" class="w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm text-slate-800 shadow-sm focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/30">
                ...
                <div>
                    ...
                    <input type="text" id="nomor_agenda" name="nomor_agenda" value="{{ old('nomor_agenda', $surat->nomor_agenda) }}" autocomplete="off" class="w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm text-slate-800 shadow-sm focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/30">
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_surat" required value="{{ old('tanggal_surat', $surat->tanggal_surat?->format('Y-m-d')) }}" class="w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm text-slate-800 shadow-sm focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/30">
                </div>
            ...
            <div class="mt-5">
                ...
                <textarea name="perihal" required rows="3" class="w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm text-slate-800 shadow-sm focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/30">{{ old('perihal', $surat->perihal) }}</textarea>
            </div>
                        ...
                        @foreach ($surat->lampiran as $file)
                            <li class="flex items-center justify-between gap-2 rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-600" :class="hapus.includes('{{ $file->id }}') ? 'opacity-50' : ''">
                                <span class="flex min-w-0 items-center gap-2">
                                    ...
                                    <a href="{{ route('lampiran.tampil', $file) }}" target="_blank" class="truncate hover:text-brand-700 hover:underline" :class="hapus.includes('{{ $file->id }}') ? 'line-through' : ''">{{ $file->nama_file }}</a>
                                </span>
            ...
            <div class="mt-5">
                ...
                <div class="rounded-lg border-2 border-dashed border-slate-300 px-4 py-6 text-center transition hover:border-brand-400">
                    <input type="file" name="lampiran[]" multiple accept=".pdf,.jpg,.jpeg,application/pdf,image/jpeg,.docx,application/vnd.openxmlformats-officedocument.wordprocessingml.document" class="w-full text-sm text-slate-500 file:mr-4 file:rounded-lg file:border-0 file:bg-brand-50 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-brand-700 hover:file:bg-brand-100">
                </div>
            </div>
        </div>

        <div class="flex items-center justify-end gap-3">
            <a href="{{ route('surat.show', $surat) }}" class="rounded-lg px-4 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-100">
                Batal
            </a>
