            ...
            <form method="POST" action="{{ route('surat.hapus') }}" x-data="{ selected: [], allIds: {{ $surat->pluck('id')->map(fn ($id) => (string) $id)->toJson() }} }">
                ...
                @if ($bisaHapusSurat)
                    <div class="mb-3 flex items-center justify-between" x-show="selected.length > 0" x-cloak>
                        ...
                        <button type="submit" onclick="return confirm('Pindahkan surat yang dipilih ke tempat sampah?')" class="inline-flex items-center gap-2 rounded-lg bg-rose-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-rose-700">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    ...
                                    <tr class="transition hover:bg-slate-50">
                                        @if ($bisaHapusSurat)
                                            <td class="px-5 py-3.5">
                                                <input type="checkbox" name="ids[]" value="{{ $item->id }}" x-model="selected" class="h-4 w-4 rounded border-slate-300 text-brand-600 focus:ring-brand-500">
                                            </td>
                                        @endif
                                        ...
                                        <td class="whitespace-nowrap px-5 py-3.5 text-right">
                                            <a href="{{ route('surat.show', $item) }}" class="inline-flex items-center gap-1 rounded-lg px-3 py-1.5 text-sm font-medium text-brand-700 transition hover:bg-brand-50">
                                                Detail
        ...
        <div class="xl:col-span-1">
            ...
            <div class="sticky top-20 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                <div class="mb-3 flex items-center justify-between">
                    ...
                    <a href="{{ route('surat.index', array_filter(['bulan' => $bulanSesudahnya]) + $filterAktif) }}" class="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-700">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
