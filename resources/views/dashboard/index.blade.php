...
@section('content')
        ...
        <div class="mb-6 overflow-hidden rounded-xl border border-amber-200 bg-amber-50 shadow-sm">
            ...
            <ul class="divide-y divide-amber-200/60">
                @foreach ($disposisiMendekati as $d)
                    ...
                    @endphp
                    <li>
                        <a href="{{ route('surat.show', $d->surat) }}"
                           class="group flex flex-col gap-1 px-5 py-3 transition hover:bg-amber-100/60 sm:flex-row sm:items-center sm:justify-between">
                            <div class="min-w-0 flex-1">
                                ...
                                <p class="mt-0.5 text-xs text-slate-500">
                                    Prioritas {{ $d->prioritas->label() }}
                                    &middot; Batas waktu {{ $d->batas_waktu->translatedFormat('d F Y') }}
                                </p>
                            </div>
    ...
    <div class="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <a href="{{ route('surat.index', ['arah' => 'masuk']) }}"
           class="rounded-xl border border-sky-200 bg-white p-5 shadow-sm transition hover:shadow">
        ...
        <a href="{{ route('surat.index', ['arah' => 'keluar']) }}"
           class="rounded-xl border border-violet-200 bg-white p-5 shadow-sm transition hover:shadow">
                ...
                @foreach ($statistikPerBulan as $bulanData)
                    ...
                    <div class="flex flex-1 flex-col items-center gap-1.5">
                        ...
                        <div class="flex w-full flex-1 items-end justify-center gap-0.5">
                            @if ($tinggiKeluar > 0)
                                <div class="w-3 rounded-t bg-violet-500" style="height: {{ $tinggiKeluar }}px"
                                     title="{{ $bulanData['label'] }}: {{ $bulanData['keluar'] }} surat keluar"></div>
                            @endif
                            @if ($tinggiMasuk > 0)
                                <div class="w-3 rounded-t bg-sky-500" style="height: {{ $tinggiMasuk }}px"
                                     title="{{ $bulanData['label'] }}: {{ $bulanData['masuk'] }} surat masuk"></div>
                            @endif
                        </div>
